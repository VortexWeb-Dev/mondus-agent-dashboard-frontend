...
import { 
  Award, ChevronUp, ChevronDown, Users, Briefcase, 
  TrendingUp, Target, BarChart3, HelpCircle
} from 'lucide-react';
...
const PerformanceReport = () => {
  ...
  if (loading || !commissionData || !performanceData) {
    return (
...
// if(!loading || commissionData.length !=0 || performanceData.length != 0){
  ...
  const currentPerformance = performanceData.performance[performanceMonth] || {};
// }
        ...
        {isExpanded && (
          <div className="pl-4 pr-2 py-2 bg-blue-200 dark:bg-blue-950/40 rounded-lg">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                { label: 'Monthly Rank', value: monthlyRank ? `#${monthlyRank}` : 'N/A' },
                { label: 'YTD Ranking', value: yearlyRank ? `#${yearlyRank}` : 'N/A' },
                { label: 'Monthly Gross Commission', value: formatCurrency(monthlyGrossCommission) },
                { label: 'YTD Gross Commission', value: formatCurrency(yearToDateCommission) }
              ].map(({ label, value }) => (
      ...
      <div className="mb-6">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-medium text-blue-900 dark:text-blue-100">Deals & Meetings</h3>
          </div>
          {isExpanded ? <ChevronUp className="h-5 w-5 text-blue-600 dark:text-blue-400" /> : <ChevronDown className="h-5 w-5 text-blue-600 dark:text-blue-400" />}
        </div>
        ...
        {isExpanded && (
          <div className="pl-4 pr-2 py-2 bg-blue-200 dark:bg-blue-950/40 rounded-lg">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                { label: 'Closed Deals', value: currentPerformance.closedDeals || 0 },
                { label: 'Active Deals', value: currentPerformance.activeDeals || 0 },
                { label: 'Unassigned Deals', value: currentPerformance.unassignedDeals || 0 },
                { label: 'Deals Without Updates', value: currentPerformance.dealsWithoutUpdates || 0 },
                { label: 'Meetings Arranged', value: currentPerformance.meetingsArranged || 0 },
                { label: 'Monthly Earnings', value: formatCurrency(currentPerformance.monthlyEarnings || 0) }
              ].map(({ label, value }) => (
                <div key={label} className="p-3 bg-blue-100 dark:bg-blue-900/20 rounded-lg">
                  <p className="text-blue-700 dark:text-blue-400 text-sm">{label}</p>
                  <p className="text-blue-900 dark:text-white text-xl font-semibold">{value}</p>
      ...
      <div className="mb-6">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-medium text-blue-900 dark:text-blue-100">Advertisements</h3>
          </div>
          {isExpanded ? <ChevronUp className="h-5 w-5 text-blue-600 dark:text-blue-400" /> : <ChevronDown className="h-5 w-5 text-blue-600 dark:text-blue-400" />}
        </div>
          ...
          <div className="pl-4 pr-2 py-2 bg-blue-200 dark:bg-blue-950/40 rounded-lg">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                { label: 'Published Ads', value: currentPerformance.publishedAds || 0 },
                { label: 'Draft Ads', value: currentPerformance.draftAds || 0 },
                { label: 'Total Worth of Ads', value: formatCurrency(currentPerformance.totalWorthOfAds || 0) }
              ].map(({ label, value }) => (
                <div key={label} className="p-3 bg-blue-100 dark:bg-blue-900/20 rounded-lg">
                  <p className="text-blue-700 dark:text-blue-400 text-sm">{label}</p>
                  <p className="text-blue-900 dark:text-white text-xl font-semibold">{value}</p>
      ...
      <div className="mb-6">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-medium text-blue-900 dark:text-blue-100">Platform Distribution</h3>
          </div>
          {isExpanded ? <ChevronUp className="h-5 w-5 text-blue-600 dark:text-blue-400" /> : <ChevronDown className="h-5 w-5 text-blue-600 dark:text-blue-400" />}
        </div>
        ...
        {isExpanded && (
          <div className="pl-4 pr-2 py-2 bg-blue-200 dark:bg-blue-950/40 rounded-lg">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                { label: 'Property Finder Ads', value: currentPerformance.pfAds || 0 },
                { label: 'Bayut Ads', value: currentPerformance.bayutAds || 0 },
                { label: 'Dubizzle Ads', value: currentPerformance.dubizzleAds || 0 },
                { label: 'Website Ads', value: currentPerformance.websiteAds || 0 }
              ].map(({ label, value }) => (
                <div key={label} className="p-3 bg-blue-100 dark:bg-blue-900/20 rounded-lg">
                  <p className="text-blue-700 dark:text-blue-400 text-sm">{label}</p>
                  <p className="text-blue-900 dark:text-white text-xl font-semibold">{value}</p>
      ...
      <div className="mt-6 p-4 bg-white dark:bg-gray-800 rounded-lg shadow-md">
        ...
        <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
          <div className="bg-gray-100 dark:bg-gray-300 py-1 px-8 rounded-full">
            <p className="text-gray-500 dark:text-gray-600 mb-1 text-lg">Total Ads</p>
            <p className="text-gray-900 dark:text-gray-800 font-bold text-xl">{currentPerformance.totalAds || 0}</p>
          </div>
          <div className="bg-gray-100 dark:bg-gray-300 py-1 px-8 rounded-full">
            <p className="text-gray-500 dark:text-gray-600 mb-1 text-lg">Monthly Rank</p>
            <p className="text-gray-900 dark:text-gray-800 font-bold text-xl">{(monthlyRank || monthlyRank==0) ? `#${monthlyRank}` : 'N/A'}</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="bg-white dark:bg-blue-950 text-blue-900 dark:text-white p-6 rounded-xl max-w-[100%] h-fit mx-auto shadow-xl">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 pb-4 border-b border-blue-300 dark:border-blue-800">
        <div>
          ...
          <p className="text-blue-600 dark:text-blue-400">{performanceMonth || currentMonth} {currentYear}</p>
        </div>
        <div className="mt-4 md:mt-0">
          <div className="bg-blue-100 dark:bg-blue-900/30 px-4 py-2 rounded-lg">
            <p className="text-blue-700 dark:text-blue-400 text-sm">Agent Name</p>
            <p className="text-lg font-medium text-blue-900 dark:text-blue-100">{JSON.parse(localStorage.getItem("user")).NAME}</p>
            <p className="text-xs text-blue-600 dark:text-blue-400">#{JSON.parse(localStorage.getItem("user")).ID}</p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="bg-blue-100 dark:bg-blue-900/20 p-4 rounded-lg">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              { icon: <Target className="h-8 w-8 text-blue-600 dark:text-blue-400 mr-3" />, label: 'Total Ads', value: currentPerformance.totalAds || 0 },
              { icon: <Award className="h-8 w-8 text-blue-600 dark:text-blue-400 mr-3" />, label: 'Gross Commission', value: formatCurrency(monthlyGrossCommission) },
              { icon: <TrendingUp className="h-8 w-8 text-blue-600 dark:text-blue-400 mr-3" />, label: 'YTD Commission', value: formatCurrency(yearToDateCommission) }
